...
async function init() {
  try {
    ...
    // 9. Auto-open restaurant modal if rank is in URL search params
    const urlParams = new URLSearchParams(window.location.search);
    const restRank = urlParams.get('restaurant');
    if (restRank) {
      const rankVal = parseInt(restRank);
      if (rankVal && state.restaurants.some(r => r.rank === rankVal)) {
        setTimeout(() => {
          window.openDetailModal(rankVal);
        }, 100);
      }
    }
...
function bindEventListeners() {
  ...
  // Jurados Modal triggers
  const btnShowJurados = document.getElementById('btn-show-jurados');
  const btnCloseJurados = document.getElementById('btn-close-jurados');
  const juradosDialog = document.getElementById('jurados-dialog');

  if (btnShowJurados && juradosDialog) {
    btnShowJurados.addEventListener('click', () => {
      juradosDialog.showModal();
    });
  }

  if (btnCloseJurados && juradosDialog) {
    btnCloseJurados.addEventListener('click', () => {
      juradosDialog.close();
    });
    juradosDialog.addEventListener('click', (e) => {
      if (e.target === juradosDialog) {
        juradosDialog.close();
      }
    });
  }
